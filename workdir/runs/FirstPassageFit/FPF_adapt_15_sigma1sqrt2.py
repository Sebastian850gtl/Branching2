...
sigmaf = lambda x : sigma*(x < 2) + sigma * np.sqrt(2)*(x>1) # diffusion d'un cluster d'une seule particule egale a 0 pour une particule plus grande que 1 (pour que le cluster au centre soit immobile)
radius_range = [0.002,0.004,0.008,0.012,0.02,0.04]
# simulation
run = bool(int(runvar))
n_sample = 500
if run:
    for r in radius_range:
        ...
        M.run(Ntmax = Ntmax,alpha_min = alpha_min,
              alpha_max = alpha_max,n_samples = n_sample,save_name = save_name,size_init = np.array([2,1]))
else:
    pass
...
fun = lambda x : -R**2* 1/(1/2 + 1)*np.log(x)
