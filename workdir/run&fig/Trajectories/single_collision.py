...
if not plot:
    ...
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    save_name = save_path +"/simtag_" +runtag
    M.run(Ntmax = Ntmax,tol = tol,n_samples = n_sample,save_name = save_name,
            stop = 1,position_init = 'center', mass_init = np.array([2,1]),save_trajectories = 1)
    ...
    for n in [5,10,20,40,50]:
        ...
        start = 5
        plt.plot(cum_n_sample[start:],cumsum_times[start:],label = r" $\varepsilon = {%s}^{-1}, \hat{\tau_{AB}}  = %.2f$" %(n,np.mean(sample_times[:,-1])))
