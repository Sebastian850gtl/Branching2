...
if not plot:
    ...
    for n in [2,4,6,10]:
        tol = 1/n
        save_path_n = save_path +"tol_1/"+ str(n)+'/tmp'
    ...
    for n in [2,4,6,10]:
        tol = 1/n
        save_path_n = save_path +"tol_1/"+ str(n)
        ...
        start = 5
        plt.plot(cum_n_sample[start:],cumsum_times[start:],label = r"tol $= \dfrac{1}{%s}, T  = %.2f$" %(n,np.mean(sample_times[:,-1])))
